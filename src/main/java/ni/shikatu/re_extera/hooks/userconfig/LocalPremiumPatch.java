...
public final class LocalPremiumPatch {
    ...
    private static String encodeField(TLRPC.TL_peerColor color) {
        return encodeTlObject(color);
    }

    private static String encodeTlObject(org.telegram.tgnet.TLObject obj) {
        if (obj == null) return null;
        try {
            int size = obj.getObjectSize();
            org.telegram.tgnet.NativeByteBuffer buffer = new org.telegram.tgnet.NativeByteBuffer(size);
            obj.serializeToStream(buffer);
            ...
            Main.log("LocalPremiumPatch: encode failed: %s", e.getMessage());
            return null;
        }
    }
    ...
    private static TLRPC.TL_emojiStatus decodeEmojiStatusField(String base64Value) {
        if (base64Value == null) return null;
        try {
            byte[] bytes = Base64.decode(base64Value, Base64.NO_WRAP);
            org.telegram.tgnet.NativeByteBuffer buffer = new org.telegram.tgnet.NativeByteBuffer(bytes.length);
            ...
            status.readParams(buffer, true);
            return status;
        } catch (Throwable e) {
            Main.log("LocalPremiumPatch: decode emoji status failed: %s", e.getMessage());
            return null;
        }
    }
    ...
    private static boolean isEmptyEmojiStatus(Object s) {
        return s == null || (s instanceof TLRPC.TL_emojiStatusEmpty);
    }
    ...
    private static void rememberFieldsIfPresent(int account, TLRPC.User user) {
        ...
        try {
            if (user.emoji_status instanceof TLRPC.TL_emojiStatus) {
                String enc = encodeTlObject((TLRPC.TL_emojiStatus) user.emoji_status);
                if (enc != null) {
                    Settings.setCachedPremiumField(account, "emoji_status", enc);
                }
            }
        } catch (Throwable ignored) {
        }
    ...
    public static void patchUser(int account, TLRPC.User user) {
        ...
        // Быстрый путь: если объект уже пропатчен (premium=true и оба поля цвета
        // не пустые), повторная работа с кэшем/SharedPreferences не нужна.
        // getUser/getCurrentUser вызываются очень часто (горячий путь UI),
        // так что этот ранний выход существенно снижает накладные расходы.
        if (user.premium && !isEmptyColor(user.color) && !isEmptyColor(user.profile_color) && !isEmptyEmojiStatus(user.emoji_status)) {
            return;
        }
        ...
        if (isEmptyEmojiStatus(user.emoji_status)) {
            TLRPC.TL_emojiStatus cached = decodeEmojiStatusField(Settings.getCachedPremiumField(account, "emoji_status"));
            if (cached != null) {
                user.emoji_status = cached;
            }
        }
    }
    ...
    /**
     * Хук на чтение, а не только на запись: putUser/putUsers/setCurrentUser
     * патчат объект только в момент, когда он туда КЛАДЁТСЯ - но если Telegram
     * подгружает пользователя в кэш в обход этих точек (например, из своей
     * локальной БД при холодном старте, до того как наши хуки успевают
     * сработать), уже закэшированный объект остаётся непатченным до
     * следующего события. Отсюда и "слетает" после перезапуска/долгого
     * простоя. Хук на сам ГЕТТЕР патчит объект при каждом чтении, независимо
     * от того, как и когда он попал в кэш - это должно закрыть все обходные
     * пути разом. afterHookedMethod, т.к. патчим уже готовый результат.
     */
    public static class GetUserHook extends XC_MethodHook {
        @Override
        public void afterHookedMethod(MethodHookParam param) {
            ...
            try {
                ...
                if (result instanceof TLRPC.User) {
                    int account = AccountUtils.getCurrentAccount(param.thisObject);
                    patchUser(account, (TLRPC.User) result);
                }
            } catch (Throwable e) {
                Main.log("LocalPremiumPatch.GetUserHook: %s", e.getMessage());
            }
        }
    }

    /** Хук на UserConfig.getCurrentUser() - ещё одна частая точка чтения "себя", используемая в UI профиля. */
    public static class GetCurrentUserHook extends XC_MethodHook {
        @Override
        public void afterHookedMethod(MethodHookParam param) {
            if (!Settings.getLocalPremium()) {
                return;
            ...
            try {
                ...
                Main.log("LocalPremiumPatch.GetCurrentUserHook: %s", e.getMessage());
            }
        }
    }
}
